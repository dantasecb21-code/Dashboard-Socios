...
export interface ComercialSalvadorDiarioResult {
...
export async function fetchComercialSalvadorDiario(): Promise<ComercialSalvadorDiarioResult> {
  try {
    const { data, error } = await supabase.functions.invoke("comercial-salvador-diario-proxy");
    ...
    console.error("Erro Comercial Salvador Diário:", err);
    return { registros: [], abas: [] };
  }
}
...
export const STATUS_COLORS: Record<DiarioStatus, string> = {
  FEITO: "hsl(142 70% 45%)",
  SEM_RESPOSTA: "hsl(48 95% 55%)",
  REAGENDAMENTO: "hsl(28 60% 40%)",
  A_FAZER: "hsl(0 0% 80%)",
};
