...
import { DiarioRecord, STATUS_LABELS } from "@/services/comercialSalvadorDiarioService";
...
export default function ComercialSalvadorDiarioExportModal({ open, onOpenChange, rows, filtroAba, filtroAgender, filtroStatus }: Props) {
  ...
  const baseName = filename.trim() || `comercial-salvador-diario${filtroAba !== "TODAS" ? `-${slug(filtroAba)}` : ""}${filtroAgender !== "TODOS" ? `-${slug(filtroAgender)}` : ""}${filtroStatus !== "TODOS" ? `-${slug(filtroStatus)}` : ""}`;
  ...
  const handleExport = async () => {
    ...
    if (format === "excel") {
      ...
      doc.setFontSize(14);
      doc.text(`Comercial Salvador — Controle Diário`, 14, 14);
      doc.setFontSize(9);
